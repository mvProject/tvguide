...
object ParserUtils {
    ...
    suspend fun loadElements(sourceUrl: String): List<Element> {
        val doc: Document = Ksoup.parseGetRequest(url = sourceUrl)

        val parsedTable = doc.select(TABLE_TAG).drop(COUNT_ONE)

        Timber.w("testing loadElements from $sourceUrl parsedTable ${parsedTable.count()}")
